...
"""
    ...
    def _Options(self, p):
        ...
        p.add_option(
            "--ignore-missing",
            action="store_true",
            help="silently skip & do not exit non-zero due missing checkouts",
        )
        ...
        p.add_option("--interactive", action="store_true", help="force interactive usage")
    ...
    def Execute(self, opt, args):
        ...
        smart_sync_manifest_path = os.path.join(self.manifest.manifestProject.worktree, smart_sync_manifest_name)
        ...
        try:
            ...
            with self.ParallelContext():
                ...
                rc = self.ExecuteInParallel(
                    opt.jobs,
                    functools.partial(self.DoWorkWrapper, mirror, opt, cmd, shell, config),
                    range(len(projects)),
