...
from .. import constants as mpm_constants
...
MAXIMUM_RETRY_SLEEP_SEC = float(os.environ.get("MPM_MAX_RETRY_SLEEP_SEC", "3600.0"))
# +-10% random jitter is added to each Fetches retry sleep duration. Configurable via env var.
RETRY_JITTER_PERCENT = float(os.environ.get("MPM_RETRY_JITTER_PERCENT", "0.1"))
...
def _run_ls_remote_with_retry(remote_url):
    ...
    - MPM_GIT_RETRY_COUNT: maximum number of attempts (default 3)
    - MPM_GIT_RETRY_DELAY: base delay in seconds before first retry (default 1)
    ...
    """
    retry_count = int(os.environ.get(mpm_constants.GIT_RETRY_COUNT_ENV_VAR, mpm_constants.GIT_RETRY_COUNT_DEFAULT))
    base_delay = int(os.environ.get(mpm_constants.GIT_RETRY_DELAY_ENV_VAR, mpm_constants.GIT_RETRY_DELAY_DEFAULT))
    ...
    for attempt in range(1, retry_count + 1):
        ...
        if any(pattern in stderr_text for pattern in mpm_constants.GIT_AUTH_ERROR_PATTERNS):
            raise ManifestInvalidRevisionError(
...
class Project:
    ...
    def _ResolveVersionConstraint(self):
        ...
        implementation in ``mpm_cli.version``.

        git ls-remote is retried up to MPM_GIT_RETRY_COUNT times on transient
    ...
    def _RemoteFetch(
        ...
        for try_n in range(retry_fetches):
            ...
            if try_n < retry_fetches - 1:
                ...
                # its upper bound are configurable via MPM_MAX_RETRY_SLEEP_SEC and
                # MPM_RETRY_JITTER_PERCENT environment variables.
