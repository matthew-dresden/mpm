...
class BaseEventLog:
    ...
    def __init__(self, env=None, repo_source_version=None, add_init_count=False):
        ...
        self._sid = f"repo-{self.start.strftime('%Y%m%dT%H%M%SZ')}-P{os.getpid():08x}"
    ...
    def Write(self, path=None):
        ...
        if path_is_socket:
            ...
            # (SOCK_DGRAM).
            print(
                "repo: warning: git trace2 logging failed: could not write to socket",
                file=sys.stderr,
            )
            return None
        ...
        # git trace2.
        try:
            with tempfile.NamedTemporaryFile(mode="xb", prefix=self._sid, dir=path, delete=False) as f:
